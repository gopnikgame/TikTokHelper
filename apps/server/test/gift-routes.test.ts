...
describe('observed gift catalogue API', () => {
  it('returns the shared gift catalogue from every workspace route', async () => {
    ...
    expect((await app.inject({ method: 'GET', url: '/api/workspaces/other/gifts' })).json())
      .toEqual([expect.objectContaining({ giftId: '5655' })]);
  });
});
