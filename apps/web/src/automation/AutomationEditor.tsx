...
export function AutomationEditor(props: Props) {
  ...
  async function resetSupporterStatistics() {
    if (!window.confirm('Сбросить статистику донатеров? Накопленные баллы и выданные уровни будут удалены без возможности восстановления. Подарки, звуки, привязки и правила останутся.')) return;
    setBusy(true);
    try {
      const result = await readJson<{ deletedSupporters: number }>(await fetch(
        `/api/workspaces/${encodeURIComponent(workspaceId)}/supporters/statistics`,
        { method: 'DELETE' },
      ));
      onNotice(result.deletedSupporters > 0
        ? `Статистика сброшена: удалено донатеров — ${result.deletedSupporters}`
        : 'Статистика уже была пустой');
    } catch {
      onNotice('Не удалось сбросить статистику донатеров. Попробуйте ещё раз.');
    } finally { setBusy(false); }
  }
  ...
  return <details className="automation-panel"><summary><span><b>Озвучивание чата и реакции</b><small>{configuration.supportLevels.length} уровней · {configuration.eventReactions.length} реакций</small></span><span>Настроить</span></summary><div className="automation-content">
    ...
    <section className="automation-section danger-zone"><header><div><p className="section-kicker">Данные рабочего пространства</p><h2>Статистика донатеров</h2></div></header><p>Удаляет накопленные баллы, статистику по эфирам и уже выданные уровни. Подарки, звуки, привязки и правила не изменятся. Новые подарки сразу начнут учитываться заново.</p><button type="button" className="danger" disabled={busy} onClick={() => void resetSupporterStatistics()}>Сбросить статистику донатеров</button></section>
  </div></details>;
}
